...
| t7|```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```| ------ |On va afficher les noms et les salaires des employés 'Hichem', 'Mohamed' et 'Maaoui' dans la session N°1.Le salaire de l'employé 'Mohamed' ne passe pas à 3800(dans la session N°1) à partir de la requête UPDATE réalisée en t4 car il n'y a pas eu de COMMIT dans la session N°2 pour valider la transaction et donc conserver la mise à jour réalisée sur le salaire de l'employé 'Mohamed'.|
