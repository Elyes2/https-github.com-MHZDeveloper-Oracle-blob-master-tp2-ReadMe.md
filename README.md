...
| t3 | ```UPDATE EMP SET SAL = SAL + 1000 WHERE ENAME ='Mohamed';```| ------ |Puisqu'il existe un lock sur la ligne concernant l'employé 'Mohamed'(réalisé à partir de la session N°2 à l'aide de la requête UPDATE),la session N°1 va rester en attente jusqu'à ce que l'interblocage soit détecté et donc les verrous sur la session N°1 seront relachés ou jusqu'à ce qu'un ordre COMMIT soit exécuté dans la session N°2 afin que le lock soit levé et ainsi une modification pourra être réalisée par le User1 relatif à la session N°1 sur les données correspondantes à l'employé 'Mohamed'|
...
| t2 | ------ |```SET TRANSACTION ISOLATION LEVEL READ COMMITTED;```|On va régler le niveau de la transaction sur READ COMMITTED dans la session N°2.<br>**NB**:Cette instruction doit être réalisée pour chaque transaction.|
...
| t11 | ```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|------|On va afficher encore une fois les salaires et les noms des employés 'Hichem','Mohamed' et 'Maaoui' dans la session N°1(On va obtenir un résultat correct car le COMMIT réalisé dans t10 a permis de relâcher les verrous sur les différentes lignes sur lesquelles on a effectué une mise à jour et donc les modifications seront conservées(On aura un résultat similaire au résultat de la requête en t9)).|
...
| t2| ------ |```SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;```|On va régler le niveau de la transaction sur Serializable dans la session N°2.<br>**NB**:Cette instruction doit être réalisée pour chaque transaction.|
| t3| ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem');```|On va afficher le nom et le salaire des employés 'Hichem' et 'Mohamed' dans la session N°2. Le salaire de l'employé 'Hichem' ne passe pas à 4000 grâce à la requête UPDATE réalisée en t1(à partir de la session N°1) car on utilise le mode Serializable dans la session N°2 qui nous fournit une isolation totale.Dans ce mode de transaction, la requête ne pourra pas voir les mises à jour réalisées par les autres transactions(tout est sauvegardé dans l'historique) sauf après la fin de l'exécution de la transaction Serializable.<br>Mohamed:2000 et Hichem:2800|
...
| t7|```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```| ------ |On va afficher les noms et les salaires des employés 'Hichem', 'Mohamed' et 'Maaoui' dans la session N°1.Le salaire de l'employé 'Mohamed' ne passe pas à 3800(dans la session N°1) à partir de la requête UPDATE réalisée en t4 car il n'y a pas eu de COMMIT dans la session N°2 pour valider la transaction et donc conserver la mise à jour réalisée sur le salaire de l'employé 'Mohamed'.<br>Mohamed:2000 Hichem:4000 Maaoui:|
| t8| ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|On va afficher maintenant les noms et les salaires des employés 'Hichem', 'Mohamed' et 'Maaoui' (dans la session N°2), on ne pourra pas voir les mises à jour réalisées par la transaction effectuée par la session N°1 car il s'agit du mode Serializable(La transaction SERIALIZABLE agit dans un environnement qui lui se présente comme si il n’y avait aucun autre user en train d'effectuer des modifications sur la base).<br>Mohamed:3800 Hichem:2800 Maaoui:|
| t9| ```Commit;``` |------|On va valider la transaction à l'aide d'un COMMIT dans la session N°1|
| t10|```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```| ------ |On va afficher dans la session N°1 les noms et les salaires des employés 'Hichem', 'Mohamed' et 'Maaoui'(Il n'y aura pas de modification lors de l'affichage par rapport au dernier ordre select réalisé pour la session N°1).|
| t11| ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|A l'aide d'un select dans la session N°2, on va effectuer la sélection des noms et des salaires tel que les noms des employés sont 'Hichem' , 'Mohamed' et 'Maaoui'.<br>Mohamed:3800 Hichem:2800 Maaoui:|
| t12| ------ | ```COMMIT;```|On va valider la transaction (Serializable) dans la session N°2(Les mises à jour effectuées par les autres transactions seront ainsi visibles).|
...
| t15| ------ |```UPDATE EMP SET SAL = 5200 WHERE ENAME ='Maaoui';```|On va tenter de modifier le salaire de l'employé 'Maaoui' à 5200 dans la session N°2 or il existe un lock sur sa ligne donc la session N°2 va rester en attente d'un ordre COMMIT dans la session N°1.|
| t16| ```COMMIT;``` |------|On valide la transaction dans la session N°1 à l'aide d'un COMMIT afin de lever le lock.La session N°2 va détecter l'interblocage ( le niveau de la transaction est Serializable et la base a été copiée dans le cache avec un lock sur la ligne de l'employé 'Maaoui').|
...
| t19| ------ |```SELECT ENAME, SAL FROM EMP WHERE ENAME IN ('Mohamed','Hichem', 'Maaoui');```|A l'aide d'une requête select, on affiche les noms et les salaires des employés 'Hichem' , 'Mohamed' et 'Maaoui'.<br>Mohamed:3800 Hichem:4000 Maaoui:5000|
